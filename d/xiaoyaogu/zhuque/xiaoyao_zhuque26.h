//xiaoyao_zhuque26.h 朱雀塔二层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_zhuque26);

virtual void create()
{
	set_name( "朱雀塔二层");
	
	add_door("朱雀塔二层5", "逍遥谷朱雀塔二层25", "逍遥谷朱雀塔二层26");
	
	add_door("朱雀塔三层", "逍遥谷朱雀塔三层31", "逍遥谷朱雀塔二层26");
...
{
	if(me->query_temp("xy/ta/朱雀/3")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷朱雀塔三层31"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/朱雀/3")!=query("count") && EQUALSTR(dir,"朱雀塔三层"))
		{
