//xiaoyao_baihu26.h 白虎塔二层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_baihu26);

virtual void create()
{
	set_name( "白虎塔二层");
	
	add_door("白虎塔二层5", "逍遥谷白虎塔二层25", "逍遥谷白虎塔二层26");
	
	add_door("白虎塔三层", "逍遥谷白虎塔三层31", "逍遥谷白虎塔二层26");
...
{
	if(me->query_temp("xy/ta/白虎/3")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷白虎塔三层31"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/白虎/3")!=query("count") && EQUALSTR(dir,"白虎塔三层"))
		{
