//xiaoyao_baihu53.h 白虎塔五层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_baihu53);

virtual void create()
{
	set_name( "白虎塔五层");
	
	add_door("白虎塔五层2", "逍遥谷白虎塔五层52", "逍遥谷白虎塔五层53");
	
	add_door("白虎塔六层", "逍遥谷白虎塔六层61", "逍遥谷白虎塔五层53");
...
{
	if(me->query_temp("xy/ta/白虎/6")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷白虎塔六层61"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/白虎/6")!=query("count") && EQUALSTR(dir,"白虎塔六层"))
		{
