//xiaoyao_zhuque17.h 朱雀塔塔底
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_zhuque17);

virtual void create()
{
	set_name( "朱雀塔塔底");
	
	add_door("朱雀塔塔底6", "逍遥谷朱雀塔塔底16", "逍遥谷朱雀塔塔底17");
	add_door("朱雀塔二层", "逍遥谷朱雀塔二层21", "逍遥谷朱雀塔塔底17");
...
{
	if(me->query_temp("xy/ta/朱雀/2")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷朱雀塔二层21"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/朱雀/2")!=query("count") && EQUALSTR(dir,"朱雀塔二层"))
		{
