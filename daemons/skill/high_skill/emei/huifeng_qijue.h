// huifeng_qijue.h 回风七绝
//code by Teapot
//date : 2001-02-12

SKILL_BEGIN(CShuifeng_qijue);
...
virtual CMapping * query_action(CChar * me)
{
	...
	struct {
		...
		{"［豆蔻青梅］：$w的剑光象绽放的春花，从记忆的深处飘来，无所迹，无所踪。", 100,25 ,100, 0,},
		{"［心有灵犀］：$w的剑光突然收敛，忽而又似穿透了时间和空间，魔幻般的直指$n的胸膛。", 180,20 , 140,60,},
		{"［金风玉露］：$w似左还右，剑光抖动处，突然幻成了两道剑影，同时分刺$n的小腹和右肩，让人无从分辨。", 260,15 ,170, 120,},
		{"［梦断巫山］：$w蓦地回缩，挽了斗大的一个剑花后，突然化轻灵为刚厉，当头疾劈而下，势重千钧！", 340,10 ,200, 180,},
		{"［万古寸灰］：$w呼啸着划出一个个剑圈，山一般压迫过来，让人无处可避。", 410,15 ,250, 240,},
		{"［生死冥河］：$w的剑光似银河飞瀑，澎湃如雪，势不可挡的向$n倾泻而来！", 500,10 ,320, 300,},
		{"［情天难补］：$w就象天边的流星划过，那么寂寥冷漠的刺向$n的胸膛，如此无助的一剑！如此伤心的一剑！如此黯然的一剑！", 700,5 ,500, 350,},
	};
	int level = 0;
	
	for(int i=7; i>0; i--)
	{
		if(action[i - 1].lvl < me->query_skill("huifeng_qijue", 1))
		{
			level = random(i);
...
virtual int valid_learn(CChar * me)
{
	...
	if ( !wizardp(me) && (DIFFERSTR(me->querystr("family/family_name"), "峨嵋派")))
		return	notify_fail("必须是峨嵋派弟子方可以学习");
	if (DIFFERSTR(me->querystr("gender"), "女性"))
		return notify_fail("回风七绝是只有女子才能练的武功。");
	if (me->query("max_mp") < 3500)
		return notify_fail("你的内力不够。");
	if (DIFFERSTR(me->querystr("family/master_id"), "zhou caiwei"))
		return notify_fail("回风七绝只能向周采薇学习。");////////////////////////////////////////////////////
...
static int do_mixhit(CChar * me, CContainer * ob)
{
	...
	if ( !wizardp(me) && (DIFFERSTR(me->querystr("family/family_name"), "峨嵋派")))
		return	notify_fail("［情天难补］必须是峨嵋派弟子方可以使用");

	if (DIFFERSTR(me->querystr("gender"), "女性"))
		return notify_fail("［情天难补］只有女子才能使用。");
	...
	if( DIFFERSTR(me->query_skill_mapped("sword"), "huifeng_qijue"))
		return notify_fail("必须配合回风剑法使用。");

	if (me->query_skill("huifeng_jian", 1) < 199)	// 200->(die)->199:Can learn.
		return notify_fail("你的回风拂柳剑火候太浅。");
	if (me->query_skill("huifeng_qijue", 1) < 320)	// 200->(die)->199:Can learn.
		return notify_fail("你的回风七绝火候太浅。");
	if (me->query_skill("linji_zhuang", 1) >= 200 && !me->Query(CChar::IS_NPC))	// 200->(die)->199:Can Learn. Appr need 200 lv.
		return notify_fail("你的临机十二庄火候太深。");
	if (me->query_skill("linji_zhuang", 1) < 199)	// 200->(die)->199:Can Learn. Appr need 200 lv.
		return notify_fail("你的临机十二庄火候太浅。");
	...
	snprintf(msg ,255,"%s终于静止，似乎在千万年的疲倦后归于无，绝望中隐藏着无限杀机。",weapon->name(1));
	message_vision(msg,me);

	i = (400 - i)/10;
	if(i < 1) i = 1;

	me->call_out(remove_effect, i, target->object_id(), i);
...
static void remove_effect(CContainer *ob1, LONG  param1, LONG amount1)
{
	...
	snprintf(msg ,255,"%s突然动了……",weapon->name(1));
	message_vision(msg, me);
