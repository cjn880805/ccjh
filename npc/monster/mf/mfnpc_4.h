...
virtual void create()
{
	static struct{
		...
		{"克拉玛", "kelama", old_man2, 0,},
		{"昆仑奴", "kunlunnu", old_man2, 8,},
		{"沙拓儿", "shatuoer", old_man2, 16,},
		{"汪佳卫", "wangjiawei", old_man2, 24,},
...
//	set("no_chan",1);
	...
	set("present_msg", "“那里走！”，$n从旁挡住$N的去路。");
	call_out(do_die, 900);	
...
void do_copyme(CChar * ob)
{
	...
	set_skill("hamagong", 50+i*(lev+query("boss")));
	set_skill("chanchu_bufa", 50+i*(lev+query("boss")));
	set_skill("shexing_diaoshou", 50+i*(lev+query("boss")));
	set_skill("lingshe_zhangfa", 50+i*(lev+query("boss")));
