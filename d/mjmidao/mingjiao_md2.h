...
//Sample for room:  明教秘道2

//coded by cat
//data: 2001-3-20

ROOM_BEGIN(CRMingJiao_md2);
...

{
	...
	CDoor * door = add_door("明教秘道7", "明教秘道7", "明教秘道2");//上
	door->set_name( "秘道");

	door = add_door("明教秘道4", "明教秘道4", "明教秘道2");//下
	door->set_name( "秘道");

	door = add_door("明教秘道5", "明教秘道5", "明教秘道2");//左
	door->set_name( "秘道");

	door = add_door("明教秘道6", "明教秘道6", "明教秘道2");//右
	door->set_name( "秘道");
...
int do_tui(CChar * me)
{
	...
	case 0://lucky
		if(query_temp("have_goods") == 1)
		{
			...
			{
			case 0:	
				message_vision ("$N扳动机关，只听一阵机关活动的声音，墙上露出一道暗门。$N闪身而入暗门自动地就关了起来!", me);
				me->move(room1);
				return 0;
			case 1:
	...
	case 1://boring
		say ("$N扳动机关，只听见沙沙一阵轻响，头上掉来了一把灰尘，就再没任何动静了。", me);
		return 0;
