...
virtual void create()
{
	static struct{
		...
		{"朱邺", "zhuye", young_man4, 0,},
		{"完颜术", "wanyanshu", young_man4, 8,},
		{"赵艾", "zhaoai", young_man4, 16,},
		{"乐莫", "lemo", young_man4, 24,},
		};
	...
	set("present_msg", "“明教弟子在此执行公务，擅闯者死！”，$n从旁挡住$N的去路。");
	call_out(do_die, 900);	
...
void do_copyme(CChar * ob)
{
	...
	set_skill("jiuyang_shengong", 50+i*(lev+query("boss")));
	set_skill("lingfa", 50+i*(lev+query("boss")));
	set_skill("shenghuo_lingfa", 50+i*(lev+query("boss")));
	set_skill("qiankun_danuoyi", 50+i*(lev+query("boss")));
	set_skill("shenghuo_xinfa", 50+i*(lev+query("boss")));
