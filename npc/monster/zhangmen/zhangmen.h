...
void die()
{
	...
	{ 
		...
		if(me->query_temp("fight/is_fighting") && is_fighting(me)!=is_fighting(this) 
			&& me->query("tradepro/index")==6
			&& EQUALSTR(me->querystr("tradepro/quest_killnpc_mp"),querystr("family/family_name")))
		{
			tell_object(me,snprintf(msg, 255,  "\n$HIW恭喜你完成了%s委托你的任务！\n",me->querystr("tradepro/quest_npc_name")));
			me->set("tradepro/quest_killnpc_ok",1);
			break;
		}
	}
	destruct(this);
