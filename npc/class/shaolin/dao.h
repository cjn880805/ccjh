...
#define SHAOLINDAO_BEGIN(npc_name) class npc_name : public CNshaolin_dao	{	public:	npc_name(){};	virtual ~npc_name(){};
#define SHAOLINDAO_END		};	

NPC_BEGIN(CNshaolin_dao);

virtual int accept_object(CChar * who, CContainer * obj)
{
	if ( who->query_temp("have_letter") && who->present("tuijian xin2") ) 
	{
	...
	if (EQUALSTR(obj->querystr("id"), "tuijian xin1" )
		&& who->query_temp("have_letter") )
	{
...
virtual void attempt_apprentice(CChar * me)
{
	...
	if ( me->query("family/generation") == query("family/generation") + 2 )
	{
		if ( me->query_temp("apprentice_ok") )
		{
			...
			say(snprintf(msg, 255, "从今以后你的法名叫做%s，恭喜你荣升为少林派清字辈弟子 !", new_name));
			command(snprintf(msg, 80, "recruit %ld", me->object_id()));
		}
		else
		{
