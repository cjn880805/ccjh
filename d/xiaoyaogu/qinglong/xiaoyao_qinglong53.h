//xiaoyao_qinglong53.h 青龙塔五层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_qinglong53);

virtual void create()
{
	set_name( "青龙塔五层");
	
	add_door("青龙塔五层2", "逍遥谷青龙塔五层52", "逍遥谷青龙塔五层53");
	
	add_door("青龙塔六层", "逍遥谷青龙塔六层61", "逍遥谷青龙塔五层53");
...
{
	if(me->query_temp("xy/ta/青龙/6")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷青龙塔六层61"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/青龙/6")!=query("count") && EQUALSTR(dir,"青龙塔六层"))
		{
