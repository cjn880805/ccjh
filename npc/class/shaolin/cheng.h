...
#define SHAOLINCHENG_BEGIN(npc_name) class npc_name : public CNshaolin_cheng	{	public:	npc_name(){};	virtual ~npc_name(){};
#define SHAOLINCHENG_END		};	

NPC_BEGIN(CNshaolin_cheng);

void attack(CChar *me)
{
	CChar::attack(me);

	if(EQUALSTR(environment()->querystr("name"), "十八罗汉阵"))
	{
		call_out(do_leave, 1);	
...
int accept_object(CChar * who, CContainer * obj)
{
	if ( who->query_temp("have_letter") && who->present("tuijian xin4") ) 
	{
...
void attempt_apprentice(CChar * me)
{
	char msg[255];
	...
	if ( me->query("family/generation") == query("family/generation") + 2 )
	{
		if ( me->query_temp("apprentice_ok") )
		{
			...
			say(snprintf(msg, 255, "从今以后你的法名叫做%s，恭喜你荣升为少林派慧字辈弟子 !", new_name));
			command(snprintf(msg, 80, "recruit %ld", me->object_id()));
		}
		else
		{
