//xiaoyao_qinglong62.h 青龙塔六层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_qinglong62);

virtual void create()
{
	set_name( "青龙塔六层");
	
	add_door("青龙塔塔顶", "逍遥谷青龙塔塔顶71", "逍遥谷青龙塔六层62");
	
	add_door("青龙塔六层", "逍遥谷青龙塔六层61", "逍遥谷青龙塔六层62");
...
{
	if(me->query_temp("xy/ta/青龙/7")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷青龙塔塔顶71"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/青龙/7")!=query("count") && EQUALSTR(dir,"青龙塔塔顶"))
		{
