...
NPC_BEGIN(CNpub3_gongpingzi);


virtual void create()
{

	set_name("公平子","gong ping zi");
...
string ask_nowcan()
{
return ("当前候选人是"+(query("candidate")->query("name"))+"\n");
}
...
int do_setname(string arg)
{
	...
	if( !dest )
		return notify_fail("公平子说：对不起，此玩家不在这里。\n");
	if (dest->is_ghost())
