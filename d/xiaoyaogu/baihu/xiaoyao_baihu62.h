//xiaoyao_baihu62.h 白虎塔六层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_baihu62);

virtual void create()
{
	set_name( "白虎塔六层");
	
	add_door("白虎塔塔顶", "逍遥谷白虎塔塔顶71", "逍遥谷白虎塔六层62");
	
	add_door("白虎塔六层", "逍遥谷白虎塔六层61", "逍遥谷白虎塔六层62");
...
{
	if(me->query_temp("xy/ta/白虎/7")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷白虎塔塔顶71"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		CContainer * ShouWei=present("kui");
		if(ShouWei)
		{
			tell_object(me, "$HIR白虎守卫大喝道，“大胆狂徒，竟敢乱闯玄武宝塔，还不快快与我退下？”");
			return 0;
		}
		if(me->query_temp("xy/ta/白虎/7")!=query("count") && EQUALSTR(dir,"白虎塔塔顶"))
		{
