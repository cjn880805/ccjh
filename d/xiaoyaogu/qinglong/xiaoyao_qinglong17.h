//xiaoyao_qinglong17.h 青龙塔塔底
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_qinglong17);

virtual void create()
{
	set_name( "青龙塔塔底");
	
	add_door("青龙塔塔底6", "逍遥谷青龙塔塔底16", "逍遥谷青龙塔塔底17");
	add_door("青龙塔二层", "逍遥谷青龙塔二层21", "逍遥谷青龙塔塔底17");
...
{
	if(me->query_temp("xy/ta/青龙/2")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷青龙塔二层21"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/青龙/2")!=query("count") && EQUALSTR(dir,"青龙塔二层"))
		{
