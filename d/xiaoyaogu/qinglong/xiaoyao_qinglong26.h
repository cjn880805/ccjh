//xiaoyao_qinglong26.h 青龙塔二层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_qinglong26);

virtual void create()
{
	set_name( "青龙塔二层");
	
	add_door("青龙塔二层5", "逍遥谷青龙塔二层25", "逍遥谷青龙塔二层26");
	
	add_door("青龙塔三层", "逍遥谷青龙塔三层31", "逍遥谷青龙塔二层26");
...
{
	if(me->query_temp("xy/ta/青龙/3")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷青龙塔三层31"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/青龙/3")!=query("count") && EQUALSTR(dir,"青龙塔三层"))
		{
