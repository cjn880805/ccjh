...
virtual void create()
{
	static struct{
		...
		{"独孤六衽", "duguliuren", taoistess, 0,},
		{"公输索", "gongshusuo", taoistess, 8,},
		{"南郭离",	"nanguoli", taoistess, 16,},
		{"公孙咎丘", "gongsunjiuqiu", taoistess, 24,},
		{"王孙须燮",	"wangsunxuxie", taoistess, 32,},
		{"南宫班",	"nangongban", taoistess, 40,},
		{"公羊度", "gongyangdu", taoistess, 48,},
		{"西门松",	"ximensong", taoistess, 56,},
		{"东园望", "dongyuanwang", taoistess, 61,},
		{"北邙歧", "beimangqi", taoistess, 66,},
		};
...
void do_copyme(CChar * ob)
{
	...
	set_skill("persuading", 50+i*3);
	set_skill("throwing", 50+i*3);
	...
	set_skill("mahayana", 50+i*3);
	set_skill("buddhism", 50+i*3);
	set_skill("jinding_zhang", 50+i*3);
	set_skill("tiangang_zhi", 50+i*3);
	set_skill("huifeng_jian", 50+i*3);
	set_skill("yanxing_dao", 50+i*3);
	set_skill("zhutian_bu", 50+i*3);
	set_skill("linji_zhuang", 50+i*3);
...
/*	CFightRoom * env;
	if( (env = (CFightRoom *)obj->present("fight_room")) )
	{
		CContainer * obj1 = env->environment();
		...
		command(snprintf(msg, 80, "join 0 %s", obj->object_id()));
	}
*/
