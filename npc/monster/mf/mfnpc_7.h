...
virtual void create()
{
	static struct{
		...
		{"张秋生", "zhangqiusheng", young_man1, 0,},
		{"黄月裳", "huangyueshang", young_man1, 8,},
		{"李无笃","liwudu", young_man1, 16,},
		{"查边簧", "zhabianhuang", young_man1, 24,},
		};
...
void do_copyme(CChar * ob)
{
	...
	set_skill("zixia_shengong", 50+i*(lev+query("boss")));
	set_skill("poyu_quan", 50+i*(lev+query("boss")));
	set_skill("huashan_sword", 50+i*(lev+query("boss")));
	set_skill("hunyuan_zhang", 50+i*(lev+query("boss")));
	set_skill("lonely_sword", 50+i*(lev+query("boss")));
	set_skill("huashan_shenfa",50+i*(lev+query("boss")));
	set_skill("literate", 50+i*(lev+query("boss")));
	set_skill("zhengqijue", 50+i*(lev+query("boss")));
