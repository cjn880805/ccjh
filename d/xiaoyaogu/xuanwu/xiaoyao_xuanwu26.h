//xiaoyao_xuanwu26.h 玄武塔二层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_xuanwu26);

virtual void create()
{
	set_name( "玄武塔二层");
	
	add_door("玄武塔二层5", "逍遥谷玄武塔二层25", "逍遥谷玄武塔二层26");
	
	add_door("玄武塔三层", "逍遥谷玄武塔三层31", "逍遥谷玄武塔二层26");
...
{
	if(me->query_temp("xy/ta/玄武塔/3")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷玄武塔三层31"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/玄武/3")!=query("count") && EQUALSTR(dir,"玄武塔三层"))
		{
