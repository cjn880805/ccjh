//xiaoyao_zhuque62.h 朱雀塔六层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_zhuque62);

virtual void create()
{
	set_name( "朱雀塔六层");
	
	add_door("朱雀塔塔顶", "逍遥谷朱雀塔塔顶71", "逍遥谷朱雀塔六层62");
	
	add_door("朱雀塔六层", "逍遥谷朱雀塔六层61", "逍遥谷朱雀塔六层62");
...
{
	if(me->query_temp("xy/ta/朱雀/7")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷朱雀塔塔顶71"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/朱雀/7")!=query("count") && EQUALSTR(dir,"朱雀塔塔顶"))
		{
