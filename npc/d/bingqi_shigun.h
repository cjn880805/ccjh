...
virtual void create()
{
	...
	set_inquiry("铸棍","师傅命我帮助别人铸棍，但到现在也不见一个人影。谁要铸棍呢？");
	set_inquiry("原料","铸棍可用寒丝羽竹，海底金母，千年神木。但效果各不一样。");
...
virtual int accept_object(CChar * player,CContainer * ob)
{
	...
	say(snprintf(msg,255,"侍棍低头检视了一下手中的%s点了点头说：。。。好我们现在就开始铸棍",ob->query("name")),player);
	say(snprintf(msg,255,"请这位%s给棍起名字吧！(zhu 棍的名字)",query_respect(player)),player);
...
int do_zhu(CChar * player, char * arg)
{
	...
	if(!arg)
		return notify_fail("侍棍认真的说：想好棍的名称再对我说。");
	sscanf(arg,"%s",w_name);
...
//	make_time=NATURE_D->game_time();
	...
	message_vision(snprintf(msg,255,"%s沉吟了一会，对侍棍悄声说了几句话。侍棍点了点头。说：好吧！",player->name()),player);
	message_vision(snprintf(msg,255,"$N双手握住一个巨大的铁棍，猛的向炉中渐渐红热起来的%s挥去！",o_name),player);
	message_vision(snprintf(msg,255,"只听得棚的一声巨响，棍头和%s粘在了一起。",o_name ),player);
	...
	message_vision("只听咯的一声轻响，一柄巨棍从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！",player);
	message_vision("侍棍见状大叫：神棍初成，人血以祭！！闪开！",player);
	message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。",player);
	message_vision("巨棍透胸穿出，侍棍惨号一声，鲜血溅得你满脸都是！",player);
	message_vision("侍棍脚下一个不稳，倒在了地上。侍棍已经奄奄一息了。",player);

	npc->set("max_hp",10);

	message_vision("巨棍又飞了起来，飞到半空，当的一声落回到地上。炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。",player);
...
//	player->set("weapon/id",id);		
	...
	player->set("weapon/type","棍");
//	player->set("weapon/time",make_time);

	club=load_item("/d/npc/m_weapon/weapon/m_club");	
	club->move(player);
	message_vision("侍棍摸起地上，带着斑斑血迹还有些烫手的巨棍，说：棍。。已。。成。。，侍棍的任务。。。也就完成了。。。。",player );
	message_vision("侍棍艰难的说：棍。。您。。收好，我该走了。。。",player);
	message_vision("侍棍说完。倏的便不见了。",player);
	npc->die();
	return 1;
}
NPC_END;
