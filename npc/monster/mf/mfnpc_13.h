...
virtual void create()
{
	static struct{
		...
		{"杨如风", "yangrufeng", taoist, 0,},
		{"柳轻风", "liuqingfeng", taoist, 8,},
		{"杜阑风", "dulanfeng", taoist, 16,},
		{"莫随风", "mosuiming", taoist, 24,},
	...
	set("present_msg", "忽然间，$N听到了一阵淡淡的萧声，似乎正是一曲“碧海生潮”。");
	call_out(do_die, 900);	
...
void do_copyme(CChar * ob)
{
	...
	set_skill("leg",50+i*(lev+query("boss")));	           
	set_skill("dodge", 50+i*(lev+query("boss")));           
	set_skill("parry", 50+i*(lev+query("boss")));            
	set_skill("sword", 50+i*(lev+query("boss")));           

	set_skill("anying_fuxiang", 50+i*(lev+query("boss")));    
	set_skill("tanzhi_shentong", 50+i*(lev+query("boss")));   
	set_skill("xuanfeng_leg", 50+i*(lev+query("boss")));      
	set_skill("luoying_shenzhang", 50+i*(lev+query("boss")));     
	set_skill("bibo_shengong", 50+i*(lev+query("boss")));     
	set_skill("luoying_shenjian", 50+i*(lev+query("boss")));  
	set_skill("yuxiao_jianfa", 50+i*(lev+query("boss")));       
	set_skill("lanhua_shou", 50+i*(lev+query("boss")));       
	set_skill("qimen_wuxing",50+i*(lev+query("boss")));       
