...
virtual void die()
{
	...
	g_Channel.do_channel(this, NULL, "wiz",  "$HIR在大家的合力围攻下，终于抢回了我们的节日礼物！");
	g_Channel.do_channel(this, NULL, "wiz",  "$HIR快乐将与大家同在！");
	g_Channel.do_channel(this, NULL, "wiz",  "$HIR《重出江湖》所有工作人员祝大家端午节日快乐！");
	set_name( "棕横天下", "zongheng tianxia");
	g_player();
	...
	for(int i=0;i<random(50)+50;i++)
	{
		bot = load_item(baowu[random(24)]);
		bot->move(obj);
	}
	CNpc::die();
}
