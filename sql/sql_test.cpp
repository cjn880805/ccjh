...
//	snprintf(szTmp, 8192, "%s", row[17]);
	if(row[17] == NULL || row[17][0] == 0 )//是新人
	{
		...
		int temp_shuxing=atoi(row[4]);
		if(temp_shuxing>0)
			me.set("str", temp_shuxing);
		else
			me.set("str", 11);
		
		temp_shuxing=atoi(row[5]);
		if(temp_shuxing>0)
			me.set("int", temp_shuxing);
		else
			me.set("int", 11);

		temp_shuxing=atoi(row[6]);
		if(temp_shuxing>0)
			me.set("con", temp_shuxing);
		else
			me.set("con", 11);

		temp_shuxing=atoi(row[7]);
		if(temp_shuxing>0)
			me.set("dex", temp_shuxing);
		else
			me.set("dex", 11);
...
{	
	...
	if (!mysql_real_connect(&chargesql, ipaddress, name, password, NULL, 0, NULL, 0))
	{
...
{
	if(me->query_temp("cmd/warning"))
	{
		CRoom * env = load_room("pk");
		me->set("cmd/warning",me->query_temp("cmd/warning"));
		me->set("cmd/time",((CContainer *) env)->query("warning"));
	}
