...
virtual void create(int nFlag = 0)		//Call it when the item is created
{
	...
	set_name( "「金蛇秘芨」下册");	//set item name
...
int do_du(CChar *me )
{
	...
	pxlevel = me->query_skill("jinshe_zhang", 1);
	if( CChar::level2skill(me->query("level")) < pxlevel)
	{
		tell_object(me,"你的实战经验不足，再怎么读也没用。");
		return 1;
	}
	if( me->query_skill("jinshe_zhang", 1) > 199)
	{
	...
	me->improve_skill("jinshe_zhang", me->query_skill("literate", 1)/3+1);
