//xiaoyao_xuanwu44.h 玄武塔四层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_xuanwu44);

virtual void create()
{
	set_name( "玄武塔四层");
	
	add_door("玄武塔四层3", "逍遥谷玄武塔四层43", "逍遥谷玄武塔四层44");
	
	add_door("玄武塔五层", "逍遥谷玄武塔五层51", "逍遥谷玄武塔四层44");
...
{
	if(me->query_temp("xy/ta/玄武/5")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷玄武塔五层51"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		CContainer * ShouWei=present("nv ba");
		if(ShouWei)
		{
			tell_object(me, "$HIR玄武守卫大喝道，“大胆狂徒，竟敢乱闯玄武宝塔，还不快快与我退下？”");
			return 0;
		}
		if(me->query_temp("xy/ta/玄武/5")!=query("count") && EQUALSTR(dir,"玄武塔五层"))
		{
