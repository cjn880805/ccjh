...
virtual void create()
{
	static struct{
		...
		{"左留篌", "zuoliuhou", old_woman, 0,},
		{"楚轻芙", "chuqingfu", old_woman, 8,},
		{"罗缚妪", "luofuyu", old_woman, 16,},
		{"唐徨妪", "tanghuangyu", old_woman, 24,},
		};
...
void do_copyme(CChar * ob)
{
	...
	set_skill("beiming_shengong", 50+i*(lev+query("boss")));
	set_skill("dodge", 50+i*(lev+query("boss")));
	set_skill("lingboweibu", 50+i*(lev+query("boss")));
	...
	set_skill("xiaoyao_dao", 50+i*(lev+query("boss")));
