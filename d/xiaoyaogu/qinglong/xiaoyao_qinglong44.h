//xiaoyao_qinglong44.h 青龙塔四层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_qinglong44);

virtual void create()
{
	set_name( "青龙塔四层");
	
	add_door("青龙塔四层3", "逍遥谷青龙塔四层43", "逍遥谷青龙塔四层44");
	
	add_door("青龙塔五层", "逍遥谷青龙塔五层51", "逍遥谷青龙塔四层44");
...
{
	if(me->query_temp("xy/ta/青龙/5")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷青龙塔五层51"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/青龙/5")!=query("count") && EQUALSTR(dir,"青龙塔五层"))
		{
