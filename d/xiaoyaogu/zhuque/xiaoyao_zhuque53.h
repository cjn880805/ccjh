//xiaoyao_zhuque53.h 朱雀塔五层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_zhuque53);

virtual void create()
{
	set_name( "朱雀塔五层");
	
	add_door("朱雀塔五层2", "逍遥谷朱雀塔五层52", "逍遥谷朱雀塔五层53");
	
	add_door("朱雀塔六层", "逍遥谷朱雀塔六层61", "逍遥谷朱雀塔五层53");
...
{
	if(me->query_temp("xy/ta/朱雀/6")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷朱雀塔六层61"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/朱雀/6")!=query("count") && EQUALSTR(dir,"朱雀塔六层"))
		{
