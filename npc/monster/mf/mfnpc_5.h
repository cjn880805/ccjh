...
virtual void create()
{
	static struct{
		...
		{"喈来食", "jielaishi", begger, 0,},
		{"介子推", "jiezitui", begger, 8,},
		{"周叔齐","zhoushuqi", begger, 16,},
		{"商伯言", "shangboyan", begger, 24,},
	...
	carry_object("zhubang")->wield();
	set("chat_chance", 5);

	set("present_msg", "突然间$N一个踉跄，原来是被一根棒子拦住了去路，$n懒洋洋的站了起来。");
	call_out(do_die, 900);	
...
void do_copyme(CChar * ob)
{
	...
	set_skill("huntian_qigong", 50+i*(lev+query("boss")));    
	set_skill("unarmed", 50+i*(lev+query("boss")));           
	set_skill("xianglong_zhang", 50+i*(lev+query("boss")));   
	set_skill("dodge", 50+i*(lev+query("boss")));      	     
	set_skill("xiaoyaoyou", 50+i*(lev+query("boss")));      
	set_skill("parry", 50+i*(lev+query("boss")));           
	set_skill("staff", 50+i*(lev+query("boss")));            
	set_skill("dagou_bang", 50+i*(lev+query("boss")));        
	set_skill("begging", 50+i*(lev+query("boss")));           
	set_skill("xunshou_shu", 50+i*(lev+query("boss")));
