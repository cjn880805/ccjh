...
ITEM_BEGIN(CIjiuyang1);

//under, we will custom our item

virtual void create(int nFlag = 0)		//Call it when the item is created
{
	set_name( "「九阳真经」第一卷");	//set item name
	...
	set("skill/exp_required",500);	// minimum combat experience required
	set("skill/jing_cost",40);	// jing cost every time study this
	set("skill/difficulty",30);	// the base int to learn this skill
	set("skill/max_skill",25);	// the maximum level you can learn
