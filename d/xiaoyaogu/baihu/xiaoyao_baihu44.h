//xiaoyao_baihu44.h 白虎塔四层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_baihu44);

virtual void create()
{
	set_name( "白虎塔四层");
	
	add_door("白虎塔四层3", "逍遥谷白虎塔四层43", "逍遥谷白虎塔四层44");
	
	add_door("白虎塔五层", "逍遥谷白虎塔五层51", "逍遥谷白虎塔四层44");
...
{
	if(me->query_temp("xy/ta/白虎/5")!=query("count") )
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				if(XyNpc->query("owner")==atoi(me->id(1)))
					XyNpc->move(load_room("逍遥谷白虎塔五层51"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		if(me->query_temp("xy/ta/白虎/5")!=query("count") && EQUALSTR(dir,"白虎塔五层"))
		{
