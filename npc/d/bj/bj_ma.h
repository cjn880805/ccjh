...
void die()
{
	...
	{
		message_vision("\n$N长恨道：君子报仇，十年不晚，后会有期！", this);
		load_room("关外中军帅帐")->set("ok",1);
...
void attack(CChar *me)
{
	...
	int idx;
	CChar * robber;
	static char qiangdao[6][20]={"bj_hao","bj_liu","bj_qiu","bj_sun","bj_tan","bj_wangchuyi"};
			...
			if(idx!=obj->query("zheng/friend1_idx")&& idx!=obj->query("zheng/friend2_idx")&&idx!=obj->query("zheng/friend3_idx"))
			{
				...
				case 0:
					...
					else
					{
						...
						me->receive_damage("hp",50+random(50));
					}
					break;
				case 1:
...
virtual char * chat_msg_combat()
{
	...
	case 2:
		...
		break;
	case 3:
		if(who->query("boss"))//如果想替换的是主阵之人，则取消
		...
		else
		{
			...
			for(idx=random(6)+1;idx<6;idx++)
			{
				if(idx!=obj->query("zheng/friend1_idx")&& idx!=obj->query("zheng/friend2_idx")&&idx!=obj->query("zheng/friend3_idx"))
				{
					...
					message_vision("$HIR马甲转身向$N说道：师弟，你来压住阵脚。", robber);
					break;
				}
			}
		}
