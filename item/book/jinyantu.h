...
ITEM_BEGIN(CIjinyantu);

//under, we will custom our item

virtual void create(int nFlag = 0)		//Call it when the item is created
{
	set_name( "金雁图谱");	//set item name

 	set_weight(800);		//set item weight

	set("unit", "本");
	set("long", "这是本很普通的书，书里面好象有一些小人飞舞的图形。");
	set("value", 0l);
	set("material", "paper");
	set("skill/name","jinyan_gong");	// name of the skill
	set("skill/exp_required",10000);	// minimum combat experience required
	set("skill/jing_cost",25);	// jing cost every time study this
	set("skill/difficulty",18);	// the base int to learn this skill
