//xiaoyao_zhuque35.h 朱雀塔三层
//秦波 2002、12、17

XiaoYao_BEGIN(CRXiaoYao_zhuque35);

virtual void create()
{
	set_name( "朱雀塔三层");
	
	add_door("朱雀塔三层4", "逍遥谷朱雀塔三层34", "逍遥谷朱雀塔三层35");
	
	add_door("朱雀塔四层", "逍遥谷朱雀塔四层41", "逍遥谷朱雀塔二层35");
...
{
	if(me->query_temp("xy/ta/朱雀/4")!=query("count") )
	{
	...
	else
	{
		...
		if(me->query_temp("xy/renwu4_1"))
		{
			...
			if(XyNpc)
			{
				...
				XyNpc->move(load_room("逍遥谷朱雀塔四层41"));
			}
		}
	}
	return 1;
...
int valid_leave(CChar * me, const char * dir)
{
	if(userp(me))
	{
		...
		CContainer * ShouWei=present("kuang niao");
		if(ShouWei)
		{
			tell_object(me, "$HIR朱雀守卫大喝道，“大胆狂徒，竟敢乱闯朱雀宝塔，还不快快与我退下？”");
			return 0;
		}
		if(me->query_temp("xy/ta/朱雀/4")!=query("count") && EQUALSTR(dir,"朱雀塔四层"))
		{
